...
field_decl	:   TIPO ( ID | ID LBRACKET INT_LITERAL RBRACKET ) (COMMA ( ID | ID LBRACKET INT_LITERAL RBRACKET ))* SEMI   { linea++; arbol.add("Declara Campos " + linea);} ;
method_decl	:   (TIPO | VOID) ID LPARENTH (TIPO ID (COMMA TIPO ID)*)? RPARENTH block     { linea++; arbol.add("Declarar Metodos " + linea);} ;
...
method_call :   method_name LPARENTH expr (COMMA expr)* RPARENTH		
				| CALLOUT LPARENTH STRING_LITERAL COMMA callout_arg (COMMA callout_arg)* RPARENTH     { linea++; arbol.add("Method_Call "+linea);} ;
...
callout_arg :   expr | STRING_LITERAL                                    { linea++; arbol.add("String_Literal "+linea);} ;
bin_op      :   ARITH_OP | REL_OP | EQ_OP | COND_OP                      { linea++; arbol.add("Op Binario "+linea);} ;
